...
def clean_df(file):
    ...
    for accent in ACCENTS:
        output_df = pd.concat([output_df, df[(df['accents']==accent)]], ignore_index=True)
    output_df.drop(['client_id', 'sentence'], axis=1, inplace=True)
    output_df['path'] = output_df["path"].str.replace(".mp3", "")
    return output_df
...
class Mfcc():
    ...
    def mp3towav(self):
        accent_df = self.df[self.df['accents']==self.accent]
        for filename in tqdm(accent_df[self.col]):
            try:
                pydub.AudioSegment.from_mp3(f"../experiments_data/mozilla/clips/{filename}.mp3").export(f"../experiments_data/mozilla/large_wavs/{filename}.wav", format="wav")
            except:
                pass
    ...
    def create_mfcc(self):
        ...
        count = 0
        for wav in accent_df[self.col]:
            if count >= self.limit:
                break
            if os.path.isfile(f"..\experiments_data\mozilla\large_wavs\{wav}.wav"):
                self.names.append(wav)
                file_name = f"..\experiments_data\mozilla\large_wavs\{wav}.wav"
...
if __name__ == '__main__':
    ACCENTS = ["Canadian English", "Australian English"]
    df=clean_df("../experiments_data/mozilla/validated.tsv")
    #print(data["accents"].value_counts())
